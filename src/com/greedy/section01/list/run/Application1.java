...
public class Application1 {
	...
	public static void main(String[] args) {
		 ...
		 * 즉, 데이터ㅡㄹㄹ 효율적으로 저장하는 자료구조와, 데이터를 처리하는 알고리즘을 미리 구현해놓은 클래스를 말한다.
		 * 
		 * 1. List 인터페이스
		 ...
		 * 배열은 크기를 벼경할 수 없고, 요소의 추가, 삭제, 정렬 등이 복잡하다는 단점을 가지고 있다.
		 * ArrayList는 크기 변경, 요소의 추가, 삭제, 정렬 기능을 미리 구현해놓고 제공해주고 있다.
		 * (자동으로 수행되는 것이지 속도가 빨라지는 것은 아니다)
		 * */
		
		/* ArrayLst는 인스턴스를 생성하게 되면 내부적으로 10칸짜리 배열을 생성해서 관리한다. */
...
//		Collection clist = new ArrayList();
		...
		/* 모든 타입들 저장 가능 */
		/* 추가 할 때는 add(method), 값을 꺼내올 때는 get(method) */
		list.add("apple");
		list.add(123);
		list.add(45.6);
		...
		list.add("apple");
		System.out.println("list : " + list); // 값을 추가
		
		list.add(1,"banana");
		System.out.println("list : " + list);    // 첫 번째 인덱스에 바나나를 넣는다.
		
		list.remove(2);
		System.out.println("list : " + list);  //2번째 인덱스에 있는 값을 제거
		
		list.set(1,  true);
		System.out.println("list : " + list); // 바나나라는 값을 true로 변경
